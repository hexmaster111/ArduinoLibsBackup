...
char Chatpad::chatpad_to_char(uint8_t chatpad_byte, uint8_t modifier) {
  ...
  if (modifier & MODIFIER_SHIFT) { //shift
    if (c >= 'a' && c <= 'z')
      return c - 32;
  }

  if (modifier & MODIFIER_LEFTCTRL) { //green key
    switch (c) {
    case 'q':
      return '!';
    case 'w':
  ...
  if (modifier & MODIFIER_RIGHTCTRL) { //orange key
    switch (c) {
    case 'r':
      return '$';
    case 'p':
 ...
 if (modifier & MODIFIER_MESSENGER){ //the people key
   switch (c){
   case 'c':
     return 0x03;
   case 'u':
